...
No idempotency key and no `Idempotency-Key` header — send the same charge twice and
you get two payments and two postings (Phase 3). No locking, no advisory locks, no
version column (Phase 4). No webhooks and no outbox (Phase 5). No refund endpoint or
logic; `refunded` is an enum value and nothing more (Phase 6).
