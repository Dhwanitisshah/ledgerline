...
**Phase 5a: Durability** — a charge that outlives the process that made it. You are
here.
...
5b** — they answer a different question (how to tell a downstream consumer that a
payment happened, without a dual-write) and they deserve their own commit rather
...
# 6. Smoke checks (another terminal)
.\scripts\smoke.ps1
...
Step 5 is optional for every smoke script except Phase 5a's, which does not need it
either — that script runs `python -m app.reconcile --once` itself, so the sweep
happens at a point in the story you can see rather than whenever a timer fires.
...
The suite runs the reconciler **in-process** rather than spawning
`python -m app.reconcile`: `sweep_once()` is a coroutine, so the tests call it
directly and assert on the `SweepReport` it returns. CI therefore never has a worker
to start, wait for, or kill — nothing hangs, and there is no polling loop deciding
how long to give a background process before failing. The separate-process entry
point is exercised by `scripts\smoke_phase5a.ps1` instead, which is the right place
for it: that is a claim about the CLI, not about the sweep.
