...
charge flow. Done.
**Phase 3: Idempotency** — a retried charge happens once. You are here.
...
No handling of simultaneous same-key requests (Phase 4). No background expiry
sweeper — expired keys are reclaimed lazily when the same key is presented again,
though `ix_idempotency_keys_expires_at` is there for when one is written. No
idempotency on any endpoint other than `POST /charges`.
